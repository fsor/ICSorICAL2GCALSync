...
function createEvent(event, calendarTz){
 ...
 if (event.hasProperty('description'))
    newEvent.summary = "Buchung " +icalEvent.description.split(' ',4).join(' ').split(': ')[1] + " | " + newEvent.summary ;

//" Zimmer: Zimmer Seeblick Vorname: Florian Nachname: Soriat Email: lfb@gmx.at Erwachsene: 2 Kinder: 0 Telefonnummer: Details: "
...
function processEventCleanup(){
  ...
  Logger.log(index);
  if(eventToDelete){
        Logger.log("Deleting canceled event " + calendarEvents[index-1].id);
        try{
          Calendar.Events.remove(targetCalendarId, calendarEvents[index-1].id);
        }
        catch (err){
          Logger.log(err);
        }
