...
var sourceCalendars = [                // The ics/ical urls that you want to get events from along with their target calendars (list a new row for each mapping of ICS url to Google Calendar)
  ...
  ["url to your calendar feed", "Calendar name"]
...
var addAlerts = false;                  // Whether to add the ics/ical alerts as notifications on the Google Calendar events, this will override the standard reminders specified by the target calendar.
...
function startSync(){
  ...
  for (var calendar of sourceCalendars){
    ...
    if(addEventsToCalendar || modifyExistingEvents || removeEventsFromCalendar){
      ...
      //loop until we received all events 
        ...
        for (var i = 0; i < calendarEvents.length; i++){
          Logger.log("Clear event" + calendarEvents[i].id);
          try{
    ...
    if (addEventsToCalendar || modifyExistingEvents){
      ...
      vevents.forEach(function(e, index){
        var eventDate = e.jCal[1][2][3].substr(0, e.jCal[1][2][3].indexOf('T'));
        //var today = new Date().toISOString().slice(0, 10);

        var ninetyDaysBeforeToday = new Date(new Date().setDate(new Date().getDate() - 90));
        Logger.log("eventDate " + new Date(eventDate) + " | " + ninetyDaysBeforeToday);
        if(new Date(eventDate) > ninetyDaysBeforeToday){
          Logger.log("this event is not older than 90 days");
        }

        if(e.jCal[1][8][3] == 'CONFIRMED' && (new Date(eventDate) > ninetyDaysBeforeToday)){ 
          processEvent(e, calendarTz);
        }
/*
          if(e.jCal[1] && e.jCal[1][8] && (e.jCal[1][8][3] == 'TENTATIVE')){ 
           processEventCleanup(e, index);
        }
        */
      });
